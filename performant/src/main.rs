...
// Config
static CHUNK_SIZE: usize = 0x1000;
...
fn main() {
    ...
    print!("available cores: {}", available_parallelism().unwrap());
    ...
    threads.push(thread::spawn(move || joiner(*cores - 1, joiner_state)));
...
fn process_chunk(buf: &[u8], records: &mut ChunkResult) {
...
fn joiner(id: usize, state: JoinerState) {
    ...
    while workers_done.load(Ordering::Relaxed) < worker_count - 1 {
        let chunk_result = state.channel.recv().unwrap();
